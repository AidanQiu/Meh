...
const context = vm.createContext({
  ...
  fetch: async (url, options) => {
    ...
    return { ok: true, json: async () => ({ version: "1.1.1", build: "1.1.1-pwa-r11" }) };
  },
...
assert(sessionData.get("meh-sw-reloaded-for-build") === "1.1.1-pwa-r4->1.1.1-pwa-r11", "reload guard was not stored for the target build");
...
for (const localLocation of [
  ...
  const localContext = vm.createContext({
    ...
    Date,
    document: {
      querySelector: () => ({ content: "1.1.1-pwa-r11" }),
    },
...
console.log("PWA r4 -> r11 simulation passed: stalled controllerchange recovered, one reload, preserved local data, safe offline fallback, no Android-local registration.");
