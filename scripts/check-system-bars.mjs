...
check(css.includes("calc(6px + var(--app-safe-bottom)) !important"), "dock surface must protect its button content internally");
check(html.includes('class="floating-dock-surface"') && html.includes('class="floating-dock-content"'), "dock position, surface, and content layers must remain separate");
...
check(app.includes("systemBarLayoutVersion: 5"), "layout settings migration must target the layered safe-area dock model");
