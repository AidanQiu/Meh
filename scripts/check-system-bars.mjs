...
check(css.includes("--dock-bottom-offset: calc(var(--dock-bottom-gap) + var(--app-safe-bottom))"), "dock must sit above the bottom safe area");
...
check(css.includes("bottom: var(--dock-bottom-offset) !important"), "dock chrome must combine the configured gap and bottom safe area");
check(css.includes("padding: 6px !important"), "bottom safe area must not become blank padding inside the dock");
...
check(app.includes("systemBarLayoutVersion: 4"), "layout settings migration must target the safe-area dock model");
