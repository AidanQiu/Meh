...
- r10 对“离底部距离”同时监听 `input` 和 `change`：前者用于拖动中的实时更新，后者覆盖部分 iOS PWA 仅在松手时提交 range 值的行为。
- 每次设置变更都会将用户的最终数值以 `!important` 直接写入 `.floating-dock` 的 `bottom`，并保留 CSS 变量作为初始布局来源。这样滑块不再依赖多条历史 CSS 规则的级联结果；0、7、40 均有可测试的明确计算值。
